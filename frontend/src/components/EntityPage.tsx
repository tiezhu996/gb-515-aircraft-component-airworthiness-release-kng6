...
export function EntityPage({ config, useStore, certificateRecords = [] }: { config: EntityConfig; useStore: EntityStore; certificateRecords?: DomainRecord[] }) {
  ...
  const createDemo = async () => {
	...
	const isRelease = config.key === 'releaseAuthorization';
	const renderPartLink = (item: DomainRecord) => {
		if (!item.relatedCode) return <span className="muted">未关联部件</span>;
		return <div className="part-link">
			<div className="part-link__row">
				<strong>{item.relatedCode}</strong>
				{item.linkedPartStatus ? <PartStatusBadge status={item.linkedPartStatus} /> : <span className="muted">部件未找到</span>}
			</div>
			{item.linkBlockReason ? <small className="part-link__block">阻塞原因：{item.linkBlockReason}</small> : null}
			{item.linkOutcome ? <small className="part-link__outcome">联动结果：{item.linkOutcome}</small> : null}
		</div>;
	};
    ...
    <section className="table-shell" aria-busy={loading}><table><thead><tr><th>编码</th><th>名称</th><th>状态</th>{isRelease && <th>部件联动</th>}<th>风险</th><th>责任人</th><th>指标</th><th>更新时间</th><th>操作</th></tr></thead><tbody>
			{items.map((item) => { const next = nextStatus(item.status, config.primaryTransitions); return <tr key={item.id}><td><strong>{item.code}</strong></td><td>{item.name}<small>{item.facility}</small></td><td>{usePartBadge ? <PartStatusBadge status={item.status}/> : <StatusBadge status={item.status}/>}</td>{isRelease && <td>{renderPartLink(item)}</td>}<td>{item.riskLevel}</td><td>{item.owner}</td><td>{item.metricValue} {item.metricUnit}</td><td>{formatDate(item.updatedAt)}</td><td>{next && canAdvance(item, next) ? <button className="table-action" onClick={() => setPending({ item, status: next })}>推进至 {next}</button> : next && requiresReviewer(item, next) && role === 'operator' ? <span className="muted">等待复核员</span> : next && !canOperate ? <span className="muted">只读</span> : <span className="muted">流程结束</span>}</td></tr>; })}
      {!items.length && !loading && <tr><td colSpan={isRelease ? 9 : 8} className="empty">暂无记录</td></tr>}
