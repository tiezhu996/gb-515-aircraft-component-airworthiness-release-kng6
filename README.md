...
- 放行授权与航空部件状态联动：提交复核与批准前按 `relatedCode` 读取部件，部件为 `hold`/`retired`（或编号不存在）时授权保持原状态与版本，返回部件编号与阻塞原因（错误码 `part_linkage_blocked`）；部件状态正常才继续双人复核。
- 批准/限制放行后，关联部件一旦进入 `hold`/`retired`，在部件迁移的同一数据库事务内把授权记为 `revoked`，并完整保留已批准版本与版本快照；乐观锁保证并发下联动只成功一次，失败回滚不留半更新。
- 证书发布同样要求 reviewer/admin，且发布者不能是当前版本的编制人。
- 证书和授权的每次创建、草稿更新与状态变化都在同一事务写入不可变版本快照和审计日志。
- 所有状态变化使用乐观锁；复核开始后业务字段锁定，防止覆盖已审证据。
- 放行页额外展示关联部件编号、部件状态、联动结果（校验通过/联动拦截/已联动撤销/未关联部件）与阻塞原因，刷新后可回读。
