...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	aircraftPartService := service.NewAircraftPartService(aircraftPartRepository, securityService, releaseAuthorizationRepository)
	...
	releaseAuthorizationService := service.NewReleaseAuthorizationService(releaseAuthorizationRepository, securityService, aircraftPartRepository)
